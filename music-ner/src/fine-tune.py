...
# Recognition in Conversational Music Recommendation Queries" accepted
# to EACL 2023
...
class DataTrainingArguments:
    ...
    task_name: Optional[str] = field(
        default="ner", metadata={"help": "The name of the task."}
    )
    dataset_name: Optional[str] = field(
        default=None, metadata={"help": "The name of the dataset to use."}
    )
    dataset_path: str = field(
        default=None, metadata={"help": "The directory path where data is."}
    )
    ...
    reinit_layers: int = field(
        default=0,
        metadata={
            "help": "Whether to re-initialize the last N Transformer blocks, where N is the argument value."
        },
    )
...
def main():
    ...
    if not isinstance(tokenizer, PreTrainedTokenizerFast):
        raise ValueError(
            ...
            "at https://huggingface.co/transformers/index.html#supported-frameworks to find the model types that meet this requirement"
        )
    ...
    def compute_metrics(p):
        """
        Compute the metrics, print and save them
        """
        ...
        final_results = compute_results(true_labels, true_predictions)
        return final_results

    # Re-initialise last layers; works only for BERT-like models
    if data_args.reinit_layers > 0:
        # if Pooler layer exists reinit it too
        encoder_temp = getattr(model, config.model_type)
        if hasattr(encoder_temp, "pooler") and encoder_temp.pooler is not None:
    ...
    if training_args.do_predict:
        logger.info("*** Predict ***")
        predictions, pred_labels, pred_metrics = trainer.predict(
        ...
        trainer.log_metrics("predict", pred_metrics)
        trainer.save_metrics("predict", pred_metrics)
        # Save predictions
